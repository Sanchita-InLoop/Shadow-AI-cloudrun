...
async function startServer() {
  ...
  function calculateUrgencyLocally(tasks: any[]) {
    const now        = Date.now();
    ...
    const mostUrgent = [...tasks].sort((a, b) => new Date(a.deadline).getTime() - new Date(b.deadline).getTime())[0];
    ...
    const proactiveAction = hasOverdue
      ? `⚠️ "${mostUrgent?.task || 'Your task'}" is OVERDUE — stop everything and handle it immediately.`
      : minGap < ONE_DAY
      ? `🚨 "${mostUrgent?.task || 'Your task'}" is due in ${hoursUntil}h — start this RIGHT NOW, skip everything else.`
      : `▶️ Start with "${mostUrgent?.task || 'Your task'}" — it's your nearest deadline. Execute step 1 immediately.`;

    return { urgencyRating, panicModeActivated, proactiveAction };
  }

  // ─── Sort tasks by true urgency priority ─────────────────────────────────────
  function sortByUrgency(tasks: any[]) {
    ...
    return [...tasks].sort((a, b) => {
      ...
      const importanceDiff = (IMPORTANCE_WEIGHT[b.importance] || 2) - (IMPORTANCE_WEIGHT[a.importance] || 2);
      return importanceDiff !== 0 ? importanceDiff : gapA - gapB;
    });
  }

  // ─── Generate 1-3 micro-tasks for ONE task, sized to actual complexity ──────
  async function generateMicroTasksForOne(task: any) {
    const now       = Date.now();
...
Return only valid JSON.`;
          ...
          properties: {
            microTasks: {
              ...
              items: {
                type: 'OBJECT' as any,
                properties: {
                  taskName:        { type: 'STRING' as any },
                  parentTaskName:  { type: 'STRING' as any },
                  durationMinutes: { type: 'NUMBER' as any },
                  focusTip:        { type: 'STRING' as any },
                },
    ...
    let microTasks = (parsed.microTasks ?? []).slice(0, 3).map((t: any) => ({
      ...t,
      parentTaskName: task.task.trim(),
      durationMinutes: Math.max(5, Math.round(Number(t.durationMinutes) || 30)),
    }));
    ...
    const total = microTasks.reduce((sum: number, t: any) => sum + t.durationMinutes, 0);
    if (total > maxTotalMinutes && total > 0) {
      const scale = maxTotalMinutes / total;
      microTasks = microTasks.map((t: any) => ({
        ...t,
  ...
  app.post('/api/rescue', async (req, res) => {
    ...
    if (!currentFocusTask?.task) {
      return res.status(400).json({ error: 'Missing currentFocusTask with "task" field.' });
    }

    const allDeadlinesArr = Array.isArray(allDeadlines) ? allDeadlines : [];
    const allTasks        = [currentFocusTask, ...allDeadlinesArr];

    // Re-sort ALL tasks by true urgency on the backend
  ...
  // Serve static UI assets or run Vite
